...
contract('ERC1400', ([address1, address2, exchange])=>{
    ...
    describe("new tokens", ()=>{
        ...
        it("emits the right data in the event", async()=>{
            ...
            mint.logs[0].args._amountIssued.toString().should.be.equal(tokens(amountToIssue).toString(), "it emitted the issued amount passed to the event")
        ...
        it("updates the balance of the new token receiver", async()=>{
            ...
            balance.toString().should.be.equal(tokens(amountToIssue).toString(), "the balance of the new tokens issued was incremented")
        })
    ...
    describe("token transfer", ()=>{
        ...
        describe("issue tokens to address 1", ()=>{
            ...
            it("updates the balance for the token receiver", async()=>{
                const addressBalance1 = await erc1400.balanceOf(address1)
                addressBalance1.toString().should.be.equal(tokens(amountToIssue).toString(), "address 1  has been issued new tokens")
            })


            it("transfers tokens to address2 from address1", async()=>{
                const transfer = await erc1400.transfer(address2, tokens(5))
                const addressBalance1 = await erc1400.balanceOf(address1)
                const addressBalance2 = await erc1400.balanceOf(address2)

                transfer.logs[0].event.should.be.equal("Transfer", "emits the Transfer event")
                transfer.logs[0].args._amount.toString().should.be.equal(tokens(5).toString(), "emits the amount transferred")
                ...
                addressBalance1.toString().should.be.equal(tokens(5).toString(), "tokens was transfered from this address")
                addressBalance2.toString().should.be.equal(tokens(5).toString(), "tokens was transfered to this address")
            })
        ...
        describe("token transfer failure", ()=>{
            ...
            it("fails due to insuffient token amount", async()=>{
                await erc1400.transfer(address2, tokens(100)).should.be.rejected
            })
    ...
    describe("token transferFrom", ()=>{
        ...
        beforeEach(async()=>{
            await erc1400.approve(exchange, tokens(10)) // approve 10 tokens to the exchange
            await erc1400.issueTokens(address1, 20) // issue tokens to address1
            transferFrom = await erc1400.transferFrom(address1, address2, tokens(10), {from: exchange}) // exchange sends tokens from address1 to address2
        })
        ...
        it("sends the token to address 2", async()=>{
            ...
            address2Balance.toString().should.be.equal(tokens(10).toString(), "the token was moved to address2 account by the exchange")
            address1Balance.toString().should.be.equal(tokens(10).toString(), "the token was moved from address account by the exchange")
        })
        ...
        it("emits the tranfer event", async()=>{
            ...
            transferFrom.logs[0].args._amount.toString().should.be.equal(tokens(10).toString(), "it emits the amount sent")
        })
